...
Write an owned copy of a library composite into `src/components/` (once, never overwriting — ejecting again errors with the file to delete first). Ejectable today: `question`, `stepper`, `timeline`, `timeline-item`, and `ui-foundation` — the library views that are compositions of primitives. TypeScript apps receive Native markup templates through `<import>` and `<use>`; Zig-core apps receive the retained compatible Zig view for `stepper` or `timeline-item`, while components without a Zig form are rejected with a clear message. Engine controls are not on the menu (theme them through [tokens](/docs/theming) instead). Each file opens with a header comment walking through the call-site migration, and each builds a widget tree identical to its library form at the moment of ejection. Unknown names get a did-you-mean plus the full ejectable list. See [Building Components](/docs/building-components#use-eject-or-build).
